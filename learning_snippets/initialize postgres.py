...
def postgre_initialize():
	...
	myCursor.execute("CREATE TABLE characters (pk_id int primary key DEFAULT nextval('characters_pk_seq'), owner_fk int references players(pk_id) NOT NULL, name text NOT NULL, current_mission int references missions(pk_id), race text NOT NULL, class text NOT NULL, level int NOT NULL);")
	...
	try:
		...
		print "Could not grant searcher proper permissions. Complain to Michael."
