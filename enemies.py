...
def get_monster_weapons_all():
    ...
    connection = psycopg2.connect("dbname=mydb user=searcher password=allDatSQL")
    myCursor = connection.cursor()
    myCursor.execute("SELECT pk_id, name, range, damage, accuracy, capacity, description, author, type, mag_cost FROM monsters_weapons ORDER BY name;")
    results = myCursor.fetchall()
    for line in results:
        weapon = {}
        weapon['pk_id'] = line[0]
...
def get_monsters_weapons(monster_id):
    ...
    connection = psycopg2.connect("dbname=mydb user=searcher password=allDatSQL")
    myCursor = connection.cursor()
    myCursor.execute("SELECT monsters_weapons.pk_id, name, range, damage, accuracy, capacity, description, monsters_weapons.author, monsters_weapons.type, monsters_weapons.mag_cost FROM monsters_weapons, monsters_weapon_map WHERE monsters_weapon_map.fk_monster_id = %s AND monsters_weapon_map.fk_weapons_id = monsters_weapons.pk_id ORDER BY name;" % monster_id)
    results = myCursor.fetchall()
    for line in results:
        weapon = {}
        weapon['pk_id'] = line[0]
...
def insert_monster_weapon(weapon):
    connection = psycopg2.connect("dbname=mydb user=searcher password=allDatSQL")
    myCursor = connection.cursor()
    wepstring = (weapon['name'], weapon['range'], weapon['damage'], weapon['toMiss'], weapon['mag'], weapon['type'], weapon['description'], weapon['author'], weapon['magCost'])
    myCursor.execute("INSERT INTO monsters_weapons (name, range, damage, accuracy, capacity, type, description, author, mag_cost) VALUES (E'%s', %s, %s, %s, %s, E'%s', E'%s', '%s', %s);" % wepstring)
    myCursor.close()
    connection.commit()
